...
# create a variable to link data in both datasets

codes = codes %>% mutate(postcode = gsub("[[:space:]]", "", pcd))

northeast = northeast %>% mutate(postcode = gsub("[[:space:]]", "", post))

# link information 

geo_rat = left_join(northeast, codes, by = "postcode")

write.csv(geo_rat, "/Users/Personas/My Cloud/side_projects/spatial_obesity/data/north_east_ratings_geolocated.csv", row.names = FALSE)
